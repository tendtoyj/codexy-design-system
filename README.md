...
| 버전 | 날짜 | 요약 |
| --- | --- | --- |
| `0.2.2` | release candidate | PDS→CDS 전환을 위한 package·registry 동기화 migration baseline |
...
| 패키지 | 역할 | 배포 채널 | 소비자 입장 |
| --- | --- | --- | --- |
| [`@tendtoyj/cds-core`](packages/cds-core) | 토큰(`@theme`), Tailwind v4 CSS entry, Pretendard, motion, utility helpers (`cn`/`tv`) | **npm** (GitHub Packages) | `pnpm add` 후 CSS/JS `import` |
...
| [`@tendtoyj/cds-ui`](packages/cds-ui) | Radix + Tailwind 컴포넌트 | **shadcn 레지스트리** (소스 복사) | `pnpm dlx shadcn`으로 파일을 소비자 레포에 복사 |
...
각 개발자는 로컬 `~/.npmrc` 또는 환경변수 `NODE_AUTH_TOKEN`으로 자기 PAT(`read:packages` scope)를 주입한다. Actions에서는 `NODE_AUTH_TOKEN` 값으로 `secrets.GITHUB_TOKEN`을 전달할 수 있다. 다른 consumer repository의 Actions가 `GITHUB_TOKEN`을 사용한다면 GitHub Packages의 package settings에서 그 repository를 **Manage Actions access**에 추가해야 한다. 상세 절차는 [PDS→CDS 0.2.2 migration guide](apps/docs/content/docs/migrations/pds-to-cds-0.2.2.mdx)를 따른다.
...
이 import가 CDS의 Tailwind v4 CSS-first 설정이다. 토큰(`--cds-color-*`, `--cds-spacing-*`, `--cds-duration-*` 등) · Pretendard · 기본 reset이 한 번에 들어온다. 별도 JavaScript preset이나 `tailwind.config.ts`는 필요하지 않다.

`cdsStylesEntry`는 같은 public export 경로를 도구에서 참조할 때 쓰는 문자열 상수다. CSS를 자동으로 import하거나 Tailwind 설정 객체를 반환하지 않는다.

```ts
import { cdsStylesEntry } from "@tendtoyj/cds-core";

console.log(cdsStylesEntry); // "@tendtoyj/cds-core/styles"
```

### 4. UI 컴포넌트 설치 (필요한 것만)

```bash
pnpm dlx shadcn@latest add https://codexy-design-system-docs.vercel.app/r/button.json
pnpm dlx shadcn@latest add https://codexy-design-system-docs.vercel.app/r/dialog.json
pnpm dlx shadcn@latest add https://codexy-design-system-docs.vercel.app/r/chat-user-message.json
```
...
`/r/*`는 최신 source를 제공하는 mutable channel이다. 재현 가능한 0.2.2 migration에는 package 버전과 함께 고정된 `/r/0.2.2/<component>.json` 경로를 사용한다.
...
1. **재실행해서 덮어쓰기** (소비자가 수정 안 했을 때)
   ```bash
   pnpm dlx shadcn@latest add https://codexy-design-system-docs.vercel.app/r/button.json --overwrite
   ```
...
→ 0.x 동안은 정확한 버전(예: `0.2.2`)으로 핀 권장. migration baseline에서는 `^`/`~` 범위를 쓰지 않는다. 1.0 이후엔 semver 안전.
...
6. release.yml 다시 발동
   ...
   → ui 는 `/r/*` latest와 `/r/<version>/*` immutable snapshot이 함께 생성됨
