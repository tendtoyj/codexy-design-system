...
- **체크 전**: 흰 바탕 + 1.5px inset 라인 (`--cds-line-normal-normal`). `box-shadow` 로 그려서 체크 전후 레이아웃 흔들림 0.
- **체크 후**: 꽉 찬 `--cds-primary-normal` + 흰 inner dot (`sm` 5px / `md` 6px).
- **Hover/Press halo**: ring 외곽 4px 확장. hover 는 `--cds-fill-normal`, press 는 `--cds-fill-strong`.
- **Focus**: visible ring 에 `outline: 2px solid var(--cds-focus-ring)` + offset 2px.
- **Invalid**: `aria-invalid="true"` 시 inset 라인 → `--cds-status-negative`.
- **Disabled**: 라디오 본체는 `opacity: 0.43`, halo 미표시. 옆 라벨 색은 컴포넌트 책임이 아니므로 컨슈머가 다음 패턴으로 함께 흐립니다(opacity 중첩을 피하려고 색만 바꿈):

```css
.row { display: flex; align-items: center; gap: 4px; }
.row:has(:disabled) > span { color: var(--cds-label-disable); }
```

## Registry 설치

```bash
npx shadcn add https://cds.codexy.com/r/radio-group
```
