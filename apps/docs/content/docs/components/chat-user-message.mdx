...
**텍스트 없는 메시지** — text 블록이 하나도 없으면 풍선 자체를 그리지 않는다. 첨부만 있는 메시지에 placeholder 를 띄우고 싶으면 앱에서 `(no message)` 같은 text 블록을 직접 넣을 것 (CDS 가 특정 언어 카피를 가지지 않게 하기 위함).

기본은 plain text 렌더. 사용자 입력은 보통 plain 이고, 마크다운을 자동 렌더하면 react-markdown 의 `<p>` 마진이 좁은 풍선 안에서 위/아래 빈 공간으로 보이기 때문에 디폴트는 끔. 굳이 마크다운으로 그리고 싶으면 `renderMarkdown` 을 명시적으로 주입 (`@tendtoyj/cds-markdown` 의 `renderMarkdown`). 자세한 가이드는 [Chat Markdown](/docs/components/chat-markdown).
