...
```tsx
import { ChatProcessTrace } from "@tendtoyj/cds-ui/components/chat-process-trace";
...
- Trigger 라벨은 진행 상태에 따라 자연 전환:
  - `isRunning={true}` → `Thinking` + 텍스트 shimmer (`.cds-animate-text-shimmer`)
...
`(1) resolveToolLabel` 사용자 정의 → `(2)` CDS 빌트인 매핑 → `(3)` 도구명 raw fallback.

CDS 빌트인은 Anthropic SDK 표준 도구 4종만 알고 있다:
...
<ChatProcessTrace
  ...
  resolveToolLabel={(name, input) => {
    ...
    // undefined 반환 → CDS 빌트인 / 도구명 raw fallback
  }}
/>
```
...
| `resolveToolLabel` | `(name, input) => string \| undefined` | — | 도구 라벨 resolver. `undefined` 반환 시 CDS 빌트인 → 도구명 fallback. |
