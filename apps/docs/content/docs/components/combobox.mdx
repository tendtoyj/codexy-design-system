---
title: Combobox
description: cmdk + Radix Popover 기반 searchable select. cmdk 는 CDS 공개 API 뒤로 완전 래핑.
---
...
1. `cmdk` 의 타입·컴포넌트(`Command`, `Command.Root`, `useCommandState` 등)는 cds-ui 에서 **재노출하지 않는다.** `combobox.tsx` 내부에만 존재.
2. prop 네이밍은 CDS 어휘(`value/onValueChange/inputValue/onInputValueChange`) 사용. cmdk 원형과의 겹침은 우연 일치로 간주.
3. 서브컴포넌트는 CDS 이름 래핑 (`ComboboxList` / `ComboboxItem` …). cmdk 의 `Command.*` 직접 export 금지.
4. **앱 스타일링은 `data-cmdk-*` 에 의존하지 말고**, CDS 가 제공하는 `data-state` / `data-selected` / `aria-selected` 만 사용.
5. 커스텀 로직(`filter` 등)은 CDS props 로 받아 내부에서 cmdk 에 전달 — passthrough 로 뚫지 않음.
...
```bash
npx shadcn add https://cds.codexy.com/r/combobox
```
