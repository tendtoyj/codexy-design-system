...
- `outlined`: 1px `--cds-line-normal-neutral` → active 시 보더만 `--cds-primary-normal` 43% 로 강조 + 텍스트 `--cds-primary-normal` (배경은 투명 유지)
...
```bash
npx shadcn add https://cds.codexy.com/r/chip
```
