...
import { Compass } from "@tendtoyj/cds-icons/icons";
...
| variant | rest | hover |
|---|---|---|
| `outlined` | border 1px `--cds-line-normal-normal` + bg 투명 | bg `--cds-fill-alternative` |
| `filled` | border 없음 + bg `--cds-fill-alternative` | bg `--cds-fill-normal` |
| `ghost` | border 없음 + bg 투명 | bg `--cds-fill-alternative` |
...
```tsx
import { ActionTile, ActionTileContent, ActionTileTitle, ActionTileDescription, ActionTileFooter } from "@tendtoyj/cds-ui/components/action-tile";
import { IconButton } from "@tendtoyj/cds-ui/components/icon-button";
import { Icon } from "@tendtoyj/cds-ui/components/icon";
import { Bookmark } from "@tendtoyj/cds-icons/icons";
