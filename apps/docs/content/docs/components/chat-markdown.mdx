---
title: Chat Markdown
description: 채팅 메시지의 markdown 렌더 가이드 — 별도 패키지 @tendtoyj/cds-markdown
---

`ChatUserMessage` · `ChatAssistantMessage` 는 모두 `renderMarkdown?: (text: string) => ReactNode` slot 을 받는다. CDS 자체는 markdown 렌더러 의존을 가지지 않고 — 권장 구현체를 별도 패키지 [`@tendtoyj/cds-markdown`](https://github.com/tendtoyj/codexy-design-system/tree/main/packages/cds-markdown) 으로 분리해 제공한다.

## 왜 별도 패키지인가

- `cds-ui` 가 `react-markdown`/`remark-gfm` 같은 무거운 deps 를 강제하지 않도록.
...
`cds-icons` 가 `cds-ui` 와 분리된 것과 동일한 원칙.

## 설치

```bash
pnpm add @tendtoyj/cds-markdown
```
...
```tsx
import { Markdown } from "@tendtoyj/cds-markdown";

<Markdown>{`**굵게** 와 \`코드\` 가 들어간 텍스트`}</Markdown>
```

루트에 `cds-markdown` 클래스가 붙으므로 앱 측에서 typography 오버라이드 가능.
...
<Markdown
  components={{
    ...
    code: ({ children }) => <code className="bg-[var(--cds-fill-alternative)] px-[4px] rounded-[4px]">{children}</code>,
  }}
>
  {text}
</Markdown>
...
`@tendtoyj/cds-markdown` 을 안 쓰고 컨슈머가 자체 함수를 만들어도 된다 (slot 시그니처만 지키면 됨).
...
CDS 본체 의존을 늘리지 않는 선에서 같은 패키지에 추가 예정인 것:
