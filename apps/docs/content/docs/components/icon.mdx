---
title: Icon
description: phosphor 래퍼 + cds-icons 커스텀 슬롯
---
...
@phosphor-icons/react 의 아이콘 컴포넌트를 `icon` prop 으로 주입하고 CDS 기본 size / color 를 적용한다. 공급자는 `@tendtoyj/cds-icons/icons` 중립 서브패스로만 소비한다(공급자 직접 import 금지).
...
```tsx
import { MyCustomIcon } from "@tendtoyj/cds-icons";
...
| `size` | `xs \| sm \| md \| lg \| xl` | `md` | CDS 5단계 |
...
```bash
npx shadcn add https://cds.codexy.com/r/icon
```
