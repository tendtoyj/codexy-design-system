...
CDS는 **모든 요소에 글로벌 squircle**을 적용한다. `border-radius` 만으로는 macOS 감성의 부드러운 모서리가 나오지 않기 때문.
...
```css
:root {
  --cds-corner-smoothing: 0.6;
}
*, *::before, *::after {
  corner-shape: squircle;                                   /* Chromium 139+ 표준 */
  -electron-corner-smoothing: var(--cds-corner-smoothing);  /* Electron 37+ */
}
```
