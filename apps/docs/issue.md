...
<DropdownMenu>
  <DropdownMenuTrigger asChild>
    <button className="cds-chat-demo-chip">…</button>
  </DropdownMenuTrigger>
...
- chip 시각 (h-22, padding/gap, transparent bg) 은 데모 전용 CSS (`.cds-chat-demo-chip`).

### 왜 임시인가

CDS 차원에서 "compact select 트리거 = chip 모양" 이라는 정의가 없는 상태.
...
다음 중 하나를 CDS 차원에서 결정해야 함.

**(가) `Select` 에 `xsmall` size + `ghost` variant 추가**
- "compact form select" 를 CDS 1급 시민으로 등록.
