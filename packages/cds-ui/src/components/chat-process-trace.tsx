...
import { CaretRight } from "@tendtoyj/cds-icons/icons";
...
const chatProcessTrace = tv({
  slots: {
    ...
    trigger: [
      ...
      "transition-colors hover:text-[color:var(--cds-label-neutral)]",
    ],
    ...
    rowHeaderInteractive: [
      ...
      "hover:text-[color:var(--cds-label-neutral)]",
    ],
    ...
    rowFailed: [
      ...
      "rounded-[4px]",
      "bg-[color:var(--cds-chat-tone-error-bg,var(--cds-fill-alternative))]",
      "text-[color:var(--cds-status-negative)]",
    ],
    rowResult: ["cds-chat-collapsible"],
    rowResultInner: [
...
> & {
  ...
  /**
   * 도구 라벨 resolver. 반환이 `undefined` 이면 CDS 빌트인(bash, text_editor,
