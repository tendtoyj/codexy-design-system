 ...
 *      윈도우 가장자리에 닿는 쪽은 OS squircle 이 처리하므로 CDS 는 신경 X."
 *
...
const appShell = tv({
  slots: {
    root: [
      ...
      "bg-[var(--cds-background-normal-alternative,var(--cds-background-normal-normal))]",
      "[--cds-app-shell-titlebar-height:44px]",
    ],
  },
});
...
interface AppShellProps extends React.ComponentPropsWithoutRef<"div"> {
   ...
   * (CDS 는 OS 직접 감지하지 않음 — Tauri API 의존성 회피.)
   */
  leftInset?: number;
...
const AppShell = React.forwardRef<HTMLDivElement, AppShellProps>(function AppShell(
  ...
  return (
    <AppShellContext.Provider value={ctxValue}>
      <div
        ref={ref}
        data-cds-component="app-shell"
        className={cn(styles.root(), className)}
        style={{
          ["--cds-app-shell-titlebar-height" as string]: `${titlebarHeight}px`,
          ...style,
        }}
        {...rest}
      >
...
const sidebarStyles = tv({
  base: [
    ...
    "bg-[var(--cds-background-normal-alternative,var(--cds-fill-normal))]",
    "transition-[width] duration-[var(--cds-duration-fast)] ease-out",
  ],
});
...
const mainStyles = tv({
  base: [
    ...
    "bg-[var(--cds-background-normal-normal)]",
    "border-l border-[var(--cds-line-normal-alternative)]",
    // 사이드바가 좌측에 *존재할 때만* 좌측 라운딩 (data-attr 로 토글)
    "data-[adjacent-back-left=true]:rounded-l-[var(--cds-radius-12)]",
  ...
  function AppShellSidePanel(
    ...
    return (
      <PanelRoleContext.Provider value="sidePanel">
        <aside
          ref={ref}
          data-cds-component="app-shell-side-panel"
          data-cds-layer="front"
          data-cds-position="trailing"
          data-state={state.open ? "open" : "closed"}
...
const splitterStyles = tv({
  base: [
    ...
    // 호버 시 살짝 강조
    "hover:border-[var(--cds-line-normal-alternative)]",
  ...
  function AppShellSplitter({ target, doubleClickResetWidth, className, ...rest }, ref) {
    ...
    return (
      ...
      <div
        ...
        data-cds-component="app-shell-splitter"
        data-cds-target={target}
...
const overlayControlsStyles = tv({
  base: [
    ...
    "h-[var(--cds-app-shell-titlebar-height,44px)]",
...
>(function AppShellLeadingControls({ className, style, ...rest }, ref) {
  ...
  return (
    <div
      ref={ref}
      data-cds-component="app-shell-leading-controls"
...
>(function AppShellTrailingControls({ className, style, ...rest }, ref) {
  ...
  return (
    <div
      ref={ref}
      data-cds-component="app-shell-trailing-controls"
      className={cn(overlayControlsStyles(), className)}
...
const panelHeaderStyles = tv({
  base: ["relative shrink-0 flex items-center", "h-[var(--cds-app-shell-titlebar-height,44px)]"],
});
...
function makePanelHeader(role: PanelRole, displayName: string) {
  const Component = React.forwardRef<HTMLDivElement, PanelHeaderBaseProps>(function PanelHeader(
    ...
    return (
      <div
        ref={ref}
        data-cds-component={`app-shell-${role}-header`}
...
const AppShellSidebarBody = React.forwardRef<HTMLDivElement, React.ComponentPropsWithoutRef<"div">>(
  function AppShellSidebarBody({ className, ...rest }, ref) {
    return (
      <div
        ref={ref}
        data-cds-component="app-shell-sidebar-body"
...
>(function AppShellSidebarFooter({ className, ...rest }, ref) {
  return (
    <div
      ref={ref}
      data-cds-component="app-shell-sidebar-footer"
...
const AppShellMainBody = React.forwardRef<HTMLDivElement, React.ComponentPropsWithoutRef<"div">>(
  function AppShellMainBody({ className, ...rest }, ref) {
    return (
      <div
        ref={ref}
        data-cds-component="app-shell-main-body"
        className={cn(panelBodyStyles(), className)}
        {...rest}
      />
    );
...
>(function AppShellSidePanelBody({ className, ...rest }, ref) {
  return (
    <div
      ref={ref}
      data-cds-component="app-shell-side-panel-body"
      className={cn(panelBodyStyles(), className)}
      {...rest}
    />
  );
