...
- 65bf874: 라이선스를 `UNLICENSED` 에서 **MIT** 로 변경.
  ...
  - 각 패키지에 `LICENSE` 동봉 — MIT (c) 2026 The Pluto + `THIRD-PARTY NOTICES` 에 Wanted Montage(WDS) 원저작권 고지(© 2026 Wanted Lab, Inc., MIT) 포함. PDS 토큰 값이 Montage 에서 이식된 부분이 있어 MIT 고지 의무를 이행하는 것.
