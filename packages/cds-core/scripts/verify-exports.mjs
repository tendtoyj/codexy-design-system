#!/usr/bin/env node
/**
 * publish 직전 가드 — cds-core 가 노출해야 하는 named export 가 빠진 채
...
try {
  ...
  console.error(`✖ cds-core dist 를 import 할 수 없음: ${distEntry}`);
...
if (missing.length) {
  console.error("✖ cds-core export 검증 실패 — 다음 named export 가 누락:");
  for (const name of missing) console.error(`    - ${name}`);
  console.error(
    "\n  → packages/cds-core/src/index.ts 에서 해당 심볼을 re-export 하고 빌드를 다시 돌려라.",
  );
  process.exit(1);
}

console.log(`✓ cds-core exports OK — ${required.length} named exports present`);
